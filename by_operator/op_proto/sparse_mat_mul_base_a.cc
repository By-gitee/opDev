...
namespace ge {
bool InferShapeAndTypeSparseMatMulBaseA(Operator& op, const string& input_name1, const string& input_name2, const string& output_name) {
...
IMPLEMT_COMMON_INFERFUNC(SparseMatMulBaseAInferShape)
{
    if(InferShapeAndTypeSparseMatMulBaseA(op, "sparse_matrix", "weight", "output")) {
        return GRAPH_SUCCESS;
    }
    return GRAPH_FAILED;
}

IMPLEMT_VERIFIER(SparseMatMulBaseA, SparseMatMulBaseAVerify)
{
...
COMMON_INFER_FUNC_REG(SparseMatMulBaseA, SparseMatMulBaseAInferShape);
VERIFY_FUNC_REG(SparseMatMulBaseA, SparseMatMulBaseAVerify);
