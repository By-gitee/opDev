#include "sparse_mat_mul_base_a.h"
namespace ge {
bool InferShapeAndTypeSparseMatMulTBaseA(Operator& op, const string& input_name1, const string& input_name2, const string& output_name) {
...
IMPLEMT_COMMON_INFERFUNC(SparseMatMulTBaseAInferShape)
{
    if(InferShapeAndTypeSparseMatMulTBaseA(op, "sparse_matrix", "weight", "output")) {
        return GRAPH_SUCCESS;
    }
    return GRAPH_FAILED;
}

IMPLEMT_VERIFIER(SparseMatMulTBaseA, SparseMatMulTBaseAVerify)
{
...
COMMON_INFER_FUNC_REG(SparseMatMulTBaseA, SparseMatMulTBaseAInferShape);
VERIFY_FUNC_REG(SparseMatMulTBaseA, SparseMatMulTBaseAVerify);
