...
namespace ge {
bool InferShapeAndTypeSparseMatMulBaseB(Operator& op, const string& input_name1, const string& input_name2, const string& output_name) {
...
IMPLEMT_COMMON_INFERFUNC(SparseMatMulBaseBInferShape)
{
    if(InferShapeAndTypeSparseMatMulBaseB(op, "sparse_matrix", "weight", "output")) {
        return GRAPH_SUCCESS;
    }
    return GRAPH_FAILED;
}

IMPLEMT_VERIFIER(SparseMatMulBaseB, SparseMatMulBaseBVerify)
{
...
COMMON_INFER_FUNC_REG(SparseMatMulBaseB, SparseMatMulBaseBInferShape);
VERIFY_FUNC_REG(SparseMatMulBaseB, SparseMatMulBaseBVerify);
