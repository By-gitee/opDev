#include "sparse_mat_mul_base_b.h"
namespace ge {
bool InferShapeAndTypeSparseMatMulTBaseB(Operator& op, const string& input_name1, const string& input_name2, const string& output_name) {
...
IMPLEMT_COMMON_INFERFUNC(SparseMatMulTBaseBInferShape)
{
    if(InferShapeAndTypeSparseMatMulTBaseB(op, "sparse_matrix", "weight", "output")) {
        return GRAPH_SUCCESS;
    }
    return GRAPH_FAILED;
}

IMPLEMT_VERIFIER(SparseMatMulTBaseB, SparseMatMulTBaseBVerify)
{
...
COMMON_INFER_FUNC_REG(SparseMatMulTBaseB, SparseMatMulTBaseBInferShape);
VERIFY_FUNC_REG(SparseMatMulTBaseB, SparseMatMulTBaseBVerify);
