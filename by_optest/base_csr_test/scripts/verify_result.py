...
MODIFIED: 2025-12-29 Updated to perform SparseMatMulTBaseCSR verification with colored output
"""
import sys
import math
import numpy as np
...
def data_contrast_with_sparse_matmul_csr(matrix_shape_file, row_ptr_file, col_indices_file, values_file, weight_file, output_file, tolerance=1e-5):
    """
    Verify the SparseMatMulTBaseCSR result: sparse_matrix (CSR format) * weight_matrix = output_matrix
    """
    ...
    K = int(matrix_shape_data[1])  # Number of columns in sparse matrix (and rows in weight matrix)
    ...
    N = len(weight_data) // K  # Number of columns in weight matrix

    # Reshape weight matrix to [K, N]
    try:
        weight_matrix = weight_data.reshape(K, N)
    except ValueError:
        print(f"Error: Cannot reshape weight matrix data to ({K}, {N}).")
        return 1

    # Initialize output matrix [M, N]
    result_computed = np.zeros((M, N), dtype=np.float32)

    # Perform CSR sparse matrix multiplication: output = sparse_matrix * weight_matrix
    ...
    for i in range(M):
        ...
        for j in range(row_start, row_end):
            ...
            for k in range(N):
                output_idx = i * N + k
                weight_idx = col_idx * N + k
                result_computed[i, k] += val * weight_matrix[col_idx, k]
    ...
    if np.allclose(result_computed, expected_output, rtol=tolerance, atol=tolerance):
        print(f"{Colors.GREEN}Verification passed: computed SparseMatMulTBaseCSR result matches expected output within tolerance.{Colors.ENDC}")
        print(f"Matrix dimensions: sparse({M}x{K}), weight({K}x{N}), output({M}x{N})")
...
if __name__ == '__main__':
    ...
    weight_file = "test_data/data/input_4.bin"        # Weight matrix [K, N]
    ...
    # Perform SparseMatMulTBaseCSR verification
    result = data_contrast_with_sparse_matmul_csr(matrix_shape_file, row_ptr_file, col_indices_file, values_file, weight_file, output_file, tolerance)
