...
{
    // define operator
    std::vector<int64_t> inputShape0{2};        // Matrix shape: [M, N] - 2 elements of int32
    std::vector<int64_t> inputShape1{65};       // Row pointer: [M+1] elements of int32 (for M=64 rows)
    std::vector<int64_t> inputShape2{2460};     // Column indices: variable length array of int32
    std::vector<int64_t> inputShape3{2460};     // Values: variable length array of float
    std::vector<int64_t> inputShape4{64, 64};   // Weight matrix: [N, K] - N=64, K=64
    std::vector<int64_t> outputShape{64, 64};   // Output matrix: [M, K] - M=64, K=64
    std::string opType = "SparseMatMulTGatherCSR";
    aclDataType dataTypeFloat = ACL_FLOAT;
    aclDataType dataTypeInt = ACL_INT32;
    aclFormat format = ACL_FORMAT_ND;
    OperatorDesc opDesc(opType);
    opDesc.AddInputTensorDesc(dataTypeInt, inputShape0.size(), inputShape0.data(), format);  // Matrix shape
    opDesc.AddInputTensorDesc(dataTypeInt, inputShape1.size(), inputShape1.data(), format);  // Row pointer
    opDesc.AddInputTensorDesc(dataTypeInt, inputShape2.size(), inputShape2.data(), format);  // Column indices
    opDesc.AddInputTensorDesc(dataTypeFloat, inputShape3.size(), inputShape3.data(), format); // Values
    opDesc.AddInputTensorDesc(dataTypeFloat, inputShape4.size(), inputShape4.data(), format); // Weight matrix
    opDesc.AddOutputTensorDesc(dataTypeFloat, outputShape.size(), outputShape.data(), format); // Output matrix
    return opDesc;
}
...
bool RunSparseMatMulTBaseCSROp(bool isDevice)
{
    // Create op desc
...
int main()
{
    ...
    if (!RunSparseMatMulTBaseCSROp(isDevice)) {
        (void) aclrtResetDevice(deviceId);
        (void) aclFinalize();
        return FAILED;
    }
