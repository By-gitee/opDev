...
OperatorDesc CreateOpDesc()
{
    ...
    std::string opType = "SparseMatMulTBaseA";
    ...
    aclopSetAttrInt(opAttr, "blockX_id", 0);
    aclopSetAttrInt(opAttr, "blockY_id", 0);
    aclopSetAttrInt(opAttr, "blockX_dim", 1);
    aclopSetAttrInt(opAttr, "blockY_dim", 1);
    aclopSetAttrInt(opAttr, "block_size", 64);
    return opDesc;
}
...
bool RunSparseMatMulTBaseAOp(bool isDevice)
{
    // Create op desc
...
int main()
{
    ...
    if (!RunSparseMatMulTBaseAOp(isDevice)) {
        (void) aclrtResetDevice(deviceId);
        (void) aclFinalize();
        return FAILED;
    }
