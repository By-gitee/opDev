...
{
    // define operator
    std::vector<int64_t> inputShape0{128};
    std::vector<int64_t> inputShape1{2460};
    std::vector<int64_t> inputShape2{64,64};
    std::vector<int64_t> inputShape3{64,64};
    std::vector<int64_t> outputShape{64,64};
    std::string opType = "SparseMatMulGatherCOO";
    aclDataType dataTypeFloat = ACL_FLOAT;
    aclDataType dataTypeInt = ACL_INT32;
    aclFormat format = ACL_FORMAT_ND;
    OperatorDesc opDesc(opType);
    opDesc.AddInputTensorDesc(dataTypeInt, inputShape0.size(), inputShape0.data(), format);
    opDesc.AddInputTensorDesc(dataTypeInt, inputShape1.size(), inputShape1.data(), format);
    opDesc.AddInputTensorDesc(dataTypeFloat, inputShape2.size(), inputShape2.data(), format);
    opDesc.AddInputTensorDesc(dataTypeFloat, inputShape3.size(), inputShape3.data(), format);
    opDesc.AddOutputTensorDesc(dataTypeFloat, outputShape.size(), outputShape.data(), format);
    return opDesc;
}
